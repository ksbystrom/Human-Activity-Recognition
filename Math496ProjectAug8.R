...
##############################################################
  ...
  #use the below line of code if you want to avoid running the nested for loops above
  #allccf = read.csv("allccf_output.csv")
...
##############################################################
  ...
  myConf = confusionMatrix(factor(train$activity), factor(as.character(train$predActivity)))
  plot(myConf$table)
  qplot(activity, as.character(predActivity), data=train,  colour= activity, geom = c("boxplot", "jitter"), main = "predicted vs. observed in validation data", xlab = "Observed Class", ylab = "Predicted Class")
##############################################################
# END OF CODE
##############################################################
